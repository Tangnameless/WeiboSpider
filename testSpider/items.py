...
class CommentItem(Item):
    ...
    comment_user = Field()     #评论用户的昵称
    content = Field()  # 评论的内容
    weibo_url = Field()  # 评论的微博的url
    created_at = Field()  # 评论发表时间
    crawl_time = Field()  # 抓取时间戳
    like = Field() #评论点赞数
    comment_to = Field() #评论的对象用户
...
class RepostItem(Item):
    ...
    """
    _id = Field()
    repost_user = Field()     #评论用户的昵称
    repost_user_id = Field()  # 评论用户的id
    content = Field()  # 转发评论的内容
    weibo_url = Field()  # 评论的微博的url
    created_at = Field()  # 评论发表时间
    crawl_time = Field()  # 抓取时间戳
    like = Field() #评论点赞数
    repost_from = Field() #转自用户
    accumulated_reposts = Field() #本条数据对应的累计转发量